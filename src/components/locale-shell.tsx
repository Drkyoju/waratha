...
export function LocaleShell({ children, locale, dict }: LocaleShellProps) {
  return (
    <RequestAccessProvider locale={locale} dict={dict}>
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:fixed focus:start-4 focus:z-[100] focus:rounded-md focus:bg-waratha-primary focus:px-4 focus:py-2 focus:text-white focus:top-[calc(var(--header-total)+0.5rem)]"
      >
